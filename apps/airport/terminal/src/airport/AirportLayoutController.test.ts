...
describe('buildAirportBootstrapLayout', () => {
	it('creates a root horizontal airport shell with Tower on the left and Briefing Room on the right', () => {
	...
	it('stacks companion panes when the terminal is narrow', () => {
		expect(resolveAirportCompanionPaneDirection(160)).toBe('down');
	...
	it('places companion panes side by side when the terminal is wide enough', () => {
		expect(resolveAirportCompanionPaneDirection(238)).toBe('right');
	});
});