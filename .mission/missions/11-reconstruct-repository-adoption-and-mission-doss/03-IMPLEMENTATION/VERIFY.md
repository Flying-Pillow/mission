...
- Slice 3, repository modes and routing:
	- `pnpm exec vitest --run packages/core/src/lib/userConfig.test.ts packages/core/src/daemon/WorkspaceManager.test.ts`
	- `userConfig` verifies the machine-local `registeredRepositories` ledger, repository registration, and repository listing behavior.
