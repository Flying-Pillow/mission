...
export class MissionDaemonService {
    private readonly runtimeLoads = new Map<string, Promise<MissionRuntimeHandle | undefined>>();
    private readonly runtimeHandles = new Map<string, MissionRuntimeHandle>();
    ...
    public async hydrateDaemonMissions(context: { surfacePath: string }): Promise<void> {
        ...
        for (const repository of await listRegisteredRepositories()) {
    ...
    public async hydrateRepositoryMissions(context: { surfacePath: string }): Promise<void> {
        ...
        await Promise.all(
            missions.map(async (mission) => {
                try {
                    await this.loadMissionRuntimeFromRegistry(
    ...
    public dispose(): void {
        for (const runtime of this.runtimeHandles.values()) {
            runtime.dispose();
        }
        this.runtimeHandles.clear();
        this.runtimeLoads.clear();
    }

    public async loadRequiredMissionRuntime(
        input: MissionIdentityPayload,
        context: { surfacePath: string },
        terminalSessionName?: string
        ...
        const runtime = await this.loadMissionRuntimeFromRegistry(payload, context, terminalSessionName);
        if (!runtime) {
            throw new Error(`Mission '${payload.missionId}' could not be resolved.`);
        }

        return this.toRequestRuntimeHandle(runtime);
    }

    public async buildMissionSnapshot(mission: MissionRuntimeHandle, missionId: string): Promise<MissionSnapshot> {
    ...
    private async loadMissionRuntimeFromRegistry(
        input: MissionIdentityPayload,
        context: { surfacePath: string },
        terminalSessionName?: string
    ): Promise<MissionRuntimeHandle | undefined> {
        const controlRoot = this.resolveControlRoot(input, context);
        const key = this.createRuntimeKey(controlRoot, input.missionId);
        const existingRuntime = this.runtimeHandles.get(key);
        if (existingRuntime) {
            return existingRuntime;
        }

        const existingLoad = this.runtimeLoads.get(key);
        if (existingLoad) {
            return existingLoad;
        }

        const loader = this.options.loadRuntime ?? this.loadMissionRuntime;
        const load = loader(
            input,
            { surfacePath: controlRoot },
            terminalSessionName
            ...
            if (!runtime) {
                this.runtimeLoads.delete(key);
                return undefined;
            }
            this.runtimeHandles.set(key, runtime);
            return runtime;
        }).catch((error) => {
            this.runtimeLoads.delete(key);
            throw error;
    ...
    private readonly loadMissionRuntime = async (
        input: MissionIdentityPayload,
        context: { surfacePath: string },
        ...
        return MissionRuntimeFactory.load(new FilesystemAdapter(controlRoot), { missionId: input.missionId }, {
            workflow,
    ...
    private toMissionEntitySnapshot(entity: MissionEntity): MissionSnapshot['mission'] {
    ...
    private createRuntimeKey(controlRoot: string, missionId: string): string {
        return `${path.resolve(controlRoot)}:${missionId}`;
    }

    private toRequestRuntimeHandle(runtime: MissionRuntimeHandle): MissionRuntimeHandle {
        return {
            clearMissionPanic: runtime.clearMissionPanic.bind(runtime),
            completeAgentSession: runtime.completeAgentSession.bind(runtime),
            completeTask: runtime.completeTask.bind(runtime),
            dispose: () => undefined,
            executeAction: runtime.executeAction.bind(runtime),
            listAvailableActionsSnapshot: runtime.listAvailableActionsSnapshot.bind(runtime),
            cancelAgentSession: runtime.cancelAgentSession.bind(runtime),
            deliver: runtime.deliver.bind(runtime),
            panicStopMission: runtime.panicStopMission.bind(runtime),
            pauseMission: runtime.pauseMission.bind(runtime),
            reopenTask: runtime.reopenTask.bind(runtime),
            restartLaunchQueue: runtime.restartLaunchQueue.bind(runtime),
            resumeMission: runtime.resumeMission.bind(runtime),
            sendAgentSessionCommand: runtime.sendAgentSessionCommand.bind(runtime),
            sendAgentSessionPrompt: runtime.sendAgentSessionPrompt.bind(runtime),
            startTask: runtime.startTask.bind(runtime),
            terminateAgentSession: runtime.terminateAgentSession.bind(runtime),
            toEntity: runtime.toEntity.bind(runtime)
        };
    }
...
export function requireMissionDaemonService(context: EntityContractContext): MissionDaemonService {
    if (!context.missionService) {
        throw new Error('Mission entity methods require a daemon-owned mission service.');
    }
    return context.missionService;
}
